...
function updateCollectionValue(discovered, cards, totalRocksMined) {
  let value = 0;

  for (const card of cards) {
    if (discovered.includes(card.number)) {
      value += rarityValues[card.rarityLevel];
    }
  }

  const valueDisplay = document.getElementById("collection-value");
  const cardLabel = totalRocksMined === 1 ? "card" : "cards";
  const pointLabel = value === 1 ? "energy point" : "energy points";

  valueDisplay.textContent = `${totalRocksMined} ${cardLabel} pulled, worth ${value} ${pointLabel}`;
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  updateCollectionValue(discovered, cards, totalRocksMined);

  cardCounts[selectedCard.number] = (cardCounts[selectedCard.number] || 0) + 1;

  //new card is discovered
  ...
  document.getElementById("reset-game").addEventListener("click", function (e) {
    ...
    updateCollectionValue(discovered, cards, totalRocksMined);
    updateEnergyDisplay(); // Also restore energy to 50
  });
